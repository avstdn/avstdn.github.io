...
var objectsArray = [];
var colorsArray = ['yellow', 'green', 'blue', 'red', 'pink', 'brown', 'turquoise', 'orange', 'purple', 'maroon'];
var clr = 0;
  ...
  if(objectsArray.length > 0) { // В массиве должно быть больше 2-х элементов
    for(var i = 0; i < objectsArray.length - 1; i++) { 
      if(isOwned(objectsArray[i].x, objectsArray[i].y, objectsArray[objectsArray.length-1].x, objectsArray[objectsArray.length-1].y)) { // Если ф-ия вернула true, то объединяем точки в кластер
        ...
        } else { // Иначе, красим все точки достигнутого кластера
          drawPoints(objectsArray[i], objectsArray[objectsArray.length-1]);
        }
      }
    }
  }
...
function setColor(obj1, obj2) {
  if(obj1.color == 'undefined') {
    clr %= 10;
    clr++;
    obj2.color = obj1.color = colorsArray[clr];
    return true;
  } else {
...
function drawPoints(obj1, obj2) {
  for (var i = 0; i < objectsArray.length - 1; i++) {
    if(obj1.color == objectsArray[i].color) {
      objectsArray[i].color = obj2.color;
    }
  }
  context.beginPath();
